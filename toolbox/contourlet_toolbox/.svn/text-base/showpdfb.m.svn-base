...
elseif ~strcmp(scaleMode,'auto1') & ~strcmp(scaleMode, 'auto2') & ~strcmp(scaleMode, 'auto3')
    display ( sprintf('Warning! There are only two scaleMode mode: auto1, auto2, auto3!\nIts defualt value is "auto2"!'));
    scaleMode = 'auto2' ;
end
...
if ~exist('lowratio', 'var')
    lowratio = 2 ;
elseif lowratio < 1
    display ( sprintf('Warning! lowratio must be larger than 1!\nIts defualt value is 2!'));
    lowratio = 2 ;
end
...
if ~exist('highratio', 'var')
    highratio = 6 ;
elseif highratio < 1
    display ( sprintf('Warning! highratio must be larger than 1!\nIts defualt value is 6!'));
    highratio = 6 ;
end
...
if ~exist('subbandgap', 'var')
    subbandgap = 1;  
elseif subbandgap < 1
    display ( sprintf('Warning! subbandgap must be no less than 1!\nIts defualt value is 1!'));
    subbandgap = 1;
end
...
if ~exist('displayMode', 'var')
    displayMode = 'hw' ;
elseif ~strcmp(displayMode,'vb') & ~strcmp(displayMode, 'vw') ...
    & ~strcmp(displayMode,'hb') & ~strcmp(displayMode, 'hw')
    display ( sprintf('Warning! There are only four display mode: "vb", "vw", "hb", "hw"!\nIts defualt value is "vb"!'));
    displayMode = 'vw' ;
end
...
elseif ~strcmp(coefMode,'real') & ~strcmp(coefMode, 'abs')
    display ( sprintf('Warning! There are only two coefficients mode: real, abs!\nIts defualt value is "abs"!'));
    coefMode = 'abs' ;
end
...
if ~isnumeric( scaleMode )
    ...
    for i = 1: nLayers
        if vScales (i, 2) < vScales (i, 1) + 1.0e-9
            error ('Error in showpdfb.m! The scale vectors are wrong!') ;
        end;
    end;
    %display ( vScales ) ;
...
if strcmp( displayMode, 'vb' ) | strcmp( displayMode, 'hb' )
...
if nWaveletsLayers > 0
    for i=2 : nWaveletsLayers + 1
        ...
        m = length(y{i});
        if m ~= 3
            error('Error in showpdfb.m! Incorect number of wavelets subbands!');  
        end
        for k = 1:m
            if isnumeric ( scaleMode ) %Keep the significant efficients
...
if strcmp( displayMode, 'vb' ) | strcmp( displayMode, 'vw' )
% Display vertically
    nHeight = size( cellLayers{1}, 1 );    
else
% Display horizontally    
    nWidth = size( cellLayers{1}, 2 ) ;
end
...
if strcmp( displayMode, 'vb' ) | strcmp( displayMode, 'vw' )
    ...
    for i = 1 : nDisplayLayers
        [h, w] = size( cellLayers{i} );
        displayIm( 1:h, nPos+1: nPos+w) = cellLayers{i};
